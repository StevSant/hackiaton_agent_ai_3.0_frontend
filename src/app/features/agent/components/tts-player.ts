...
@Component({
  ...
  template: `
    <div
      class="flex items-center gap-2.5 flex-wrap px-3 py-2 rounded-lg border border-line bg-soft text-ink-2"
    >
      <button
        type="button"
      ...
      <input
        type="range"
        class="flex-1 min-w-[120px] accent-brand cursor-pointer"
        min="0"
        max="100"
        step="1"
        ...
        (input)="onSeek($event)"
      />
      <span class="text-[11px] tabular-nums text-ink-3 w-9 text-right">{{ progressPct() }}%</span>
      ...
      @if (voices().length > 0) {
        <label class="flex items-center gap-1 text-[11px] text-ink-3 max-w-[180px]">
          <ui-icon name="record_voice_over" [size]="14" />
          <select
            class="bg-surface border border-line rounded px-1 py-0.5 text-[11px] text-ink truncate focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
            aria-label="Voz"
