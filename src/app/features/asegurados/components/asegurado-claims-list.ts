...
@Component({
  selector: 'asegurado-claims-list',
  standalone: true,
      ...
      <table class="w-full text-[13px] border-collapse">
        <thead>
          <tr class="bg-soft">
            ...
            <th class="text-left font-medium text-ink-3 text-[11.5px] tracking-wide py-2.5 px-3 border-b border-line">Proveedor</th>
        ...
        <tbody>
          @for (c of claims(); track c.id) {
            <tr class="cursor-pointer hover:bg-soft transition-colors" (click)="open.emit(c.id)">
              ...
              <td class="px-3 py-3 border-b border-line align-middle text-ink-2">
                {{ c.proveedor ?? '—' }}
              </td>
...
export class AseguradoClaimsList {
